...
class AuditLog(DBBase):
    ...
    grant_record_uuid: Mapped[str] = Column(String(36), comment="授权记录唯一标识")
    create_time: Mapped[datetime] = Column(DateTime, default=datetime.now, comment="创建时间")

    def __repr__(self):
        return f"<AuditLog(id={self.id}, uuid={self.uuid}, capsule_uuid={self.capsule_uuid}, grant_record_uuid={self.grant_record_uuid}, create_time={self.create_time})>"
    ...
    def to_dict(self):
        return {
            ...
            "grant_record_uuid": self.grant_record_uuid,
