...
class CapsuleScope(BaseModel):
    basic_data: List[str] = Field(default=None, description="基础数据维度")
    zkp_data: List[str] = Field(default=None, description="ZKP数据指标")
...
class CapsuleClaimModel(BaseModel):
    ...
    scope: Optional[CapsuleScope] = Field(default=None, description="数据范围")
