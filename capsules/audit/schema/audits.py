...
class AuditLog(DBBase):
    ...
    action: Mapped[str] = Column(String(20), comment="数据胶囊操作行为")
    description: Mapped[str] = Column(String(255), comment="数据胶囊操作描述")
    status: Mapped[str] = Column(String(20), comment="数据胶囊操作状态")
    ...
    def __repr__(self):
        return f"<AuditLog(id={self.id}, uuid={self.uuid}, action={self.action}, description={self.description}, status={self.status}, capsule_uuid={self.capsule_uuid}, grant_record_uuid={self.claim_uuid}, create_time={self.create_time})>"
